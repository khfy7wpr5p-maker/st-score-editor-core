...
if (address === null || typeof address === 'string') { server.close(); throw new Error('APP-09B WebKit server did not expose a TCP port.'); }
...
try {
  ...
  await page.waitForFunction(() => ['true', 'false'].includes(document.documentElement.dataset.app09bRendererReady ?? ''), null, { timeout: 30_000 });
  ...
  console.log(`APP-09B WebKit automatic evidence: ${JSON.stringify(automatic)}`);

  let forcedRenderError = null;
  if (automatic.svgCount < 1) {
    forcedRenderError = await page.evaluate(async () => {
      try {
        const controller = globalThis.STScoreEditorAppController;
        if (!controller || typeof controller.renderCurrent !== 'function') return 'RENDER_CURRENT_UNAVAILABLE';
  ...
  console.log(`APP-09B WebKit forced evidence: ${JSON.stringify({ forcedRenderError, ...forced })}`);

  if (automatic.svgCount < 1) {
    const diagnostics = consoleErrors.length === 0 ? 'none' : consoleErrors.slice(-12).join(' | ');
    throw new Error(`APP-09B automatic render missing; forcedRenderError=${forcedRenderError}; forcedSvgCount=${forced.svgCount}; automatic=${JSON.stringify(automatic)}; forced=${JSON.stringify(forced)}; console=${diagnostics}`);
  }
  if (automatic.rendererReady !== 'true' || automatic.renderStatus !== 'current') throw new Error(`APP-09B render evidence incomplete: ${JSON.stringify(automatic)}`);
