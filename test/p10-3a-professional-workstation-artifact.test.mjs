...
test('P10-3A optional workstation artifact is independently bounded and preserves P10-1/P10-2 artifacts',async()=>{
  for(const script of [
  ...
  const manifest=await readJson('dist/browser/st-score-editor-p10-3a-workstation.manifest.json');
  const bundle=await readFile('dist/browser/st-score-editor-p10-3a-workstation.js');
  const html=await readFile('dist/browser/st-score-editor-p10-3a-workstation.html','utf8');
