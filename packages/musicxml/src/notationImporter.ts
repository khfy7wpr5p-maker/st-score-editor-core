...
const parseNoteNotation=(note:ParsedXmlNode):Readonly<NoteNotation>=>{
  const notations=one(note,'notations','notations');
  const ties=boundaryMarks(notations,'tied');
  const slurs=boundaryMarks(notations,'slur');
  const plain=children(note,'tie').map((item)=>attr(item,'type'));
  const plainTypes=[...plain].sort();const tiedTypes=[...ties.map((item)=>item.type)].sort();
...
const parseBeams=(note:ParsedXmlNode):readonly BeamSpec[]=>{
  ...
  const beams=children(note,'beam').map((node,index)=>{
...
const parseTuplet=(note:ParsedXmlNode):TupletSpec|null=>{
  const time=one(note,'time-modification','time-modification');
  const notations=one(note,'notations','notations');
  const marks=boundaryMarks(notations,'tuplet');
  ...
  const actualNode=one(time,'actual-notes','actual-notes',true);const normalNode=one(time,'normal-notes','normal-notes',true);
  if(actualNode===null||normalNode===null)throw new MusicXmlError('time-modification is incomplete.','INVALID_MUSICXML_SEMANTICS');
  const actualNotes=positive(actualNode,'actual-notes',32);const normalNotes=positive(normalNode,'normal-notes',32);
  return Object.freeze({actualNotes,normalNotes,marks});
};

const parseEventNotation=(note:ParsedXmlNode):Readonly<EventNotation>=>{
  const dots=children(note,'dot').length;if(dots>3)throw new MusicXmlError('dot count exceeds admitted range.','INVALID_MUSICXML_SEMANTICS',{dots});
  return Object.freeze({dots,beams:parseBeams(note),tuplet:parseTuplet(note)});
};
...
const parseBars=(measure:ParsedXmlNode):readonly BarlineSpec[]=>{
  const seen=new Set<string>();const bars=children(measure,'barline').map((bar,index)=>{
    ...
    const styleNode=one(bar,'bar-style','bar-style',true);if(styleNode===null)throw new MusicXmlError('barline style is required.','INVALID_MUSICXML_SEMANTICS');const style=requiredText(styleNode,'bar-style');
    if(!['regular','light-light','light-heavy','heavy-light','heavy-heavy','dashed','dotted','none'].includes(style))throw new MusicXmlError('barline style is unsupported.','UNSUPPORTED_MUSICXML',{style});
...
const noteStream=(note:ParsedXmlNode):{staff:number;voice:number}=>{
  const staffNode=one(note,'staff','staff'),voiceNode=one(note,'voice','voice');return{staff:staffNode===null?1:positive(staffNode,'staff',128),voice:voiceNode===null?1:positive(voiceNode,'voice',1024)};
};

const buildNotation=(score:ScoreDocument,root:ParsedXmlNode):Readonly<NotationDocument>=>{
  const measureEntries:Array<{target:ReturnType<typeof addressEntity>;notation:MeasureNotation}>=[];
  const eventMap=new Map<string,EventNotation>();
  ...
  for(const [partIndex,partNode] of partNodes.entries()){
    const p=partIndex+1;const part=score.parts[partIndex]!;const measures=children(partNode,'measure');const reference=part.staves[0]?.measures??[];
    ...
    for(const [measureIndex,measureNode] of measures.entries()){
      const m=measureIndex+1;const attributes=one(measureNode,'attributes','attributes');const key=parseKey(attributes),time=parseTime(attributes),clefs=parseClefs(attributes,part.staves.length),barlines=parseBars(measureNode);
      for(let staff=1;staff<=part.staves.length;staff+=1){const target=addressEntity(score,`measure-${p}-${staff}-${m}`);if(target.kind!=='measure')throw new MusicXmlError('Imported measure address is invalid.','INVALID_MUSICXML_SEMANTICS');const notation:Object.freeze extends never?never:MeasureNotation=Object.freeze({timeSignature:time,keySignature:key,clef:clefs.get(staff)??null,barlines});if(time!==null||key!==null||notation.clef!==null||barlines.length>0)measureEntries.push({target,notation});}
      const eventCounters=new Map<string,number>();const noteCounters=new Map<string,number>();
      for(const noteNode of children(measureNode,'note')){
        const stream=noteStream(noteNode);if(stream.staff>part.staves.length)throw new MusicXmlError('note staff exceeds imported part staves.','INVALID_MUSICXML_SEMANTICS',{stream});const keyStream=`${stream.staff}:${stream.voice}`;const chord=one(noteNode,'chord','chord')!==null;
        let eventIndex=eventCounters.get(keyStream)??0;if(!chord){eventIndex+=1;eventCounters.set(keyStream,eventIndex);noteCounters.set(keyStream,0);}else if(eventIndex===0){throw new MusicXmlError('chord marker has no preceding event in stream.','INVALID_MUSICXML_SEMANTICS',{stream});}
        const eventId=`event-${p}-${stream.staff}-${m}-${stream.voice}-${eventIndex}`;const targetEvent=addressEntity(score,eventId);if(targetEvent.kind!=='event')throw new MusicXmlError('Notation event did not resolve in imported score.','INVALID_MUSICXML_SEMANTICS',{eventId});
        const eventNotation=parseEventNotation(noteNode);const existingEvent=eventMap.get(eventId);if(existingEvent!==undefined&&!same(existingEvent,eventNotation))throw new MusicXmlError('Chord tones disagree on event-level notation.','INVALID_MUSICXML_SEMANTICS',{eventId});if(existingEvent===undefined&&!defaultEvent(eventNotation))eventMap.set(eventId,eventNotation);
        ...
        const nextNote=(noteCounters.get(keyStream)??0)+1;noteCounters.set(keyStream,nextNote);const noteId=`note-${p}-${stream.staff}-${m}-${stream.voice}-${eventIndex}-${nextNote}`;const targetNote=addressEntity(score,noteId);if(targetNote.kind!=='note')throw new MusicXmlError('Notation note did not resolve in imported score.','INVALID_MUSICXML_SEMANTICS',{noteId});const noteNotation=parseNoteNotation(noteNode);if(!defaultNote(noteNotation))noteMap.set(noteId,noteNotation);
      }
    }
  }
  const measures=measureEntries.map((entry)=>{if(entry.target.kind!=='measure')throw new MusicXmlError('Measure target kind changed.','INVALID_MUSICXML_SEMANTICS');return{target:entry.target,notation:entry.notation};});
  const events=[...eventMap].map(([id,notation])=>{const target=addressEntity(score,id);if(target.kind!=='event')throw new MusicXmlError('Event target kind changed.','INVALID_MUSICXML_SEMANTICS',{id});return{target,notation};});
  const notes=[...noteMap].map(([id,notation])=>{const target=addressEntity(score,id);if(target.kind!=='note')throw new MusicXmlError('Note target kind changed.','INVALID_MUSICXML_SEMANTICS',{id});return{target,notation};});
  return createNotationDocument(score,{contractVersion:'1.0.0',documentId:score.id,revisionId:score.revision.id,measures,events,notes});
};
...
export const importNotationMusicXml=(input:MusicXmlInput,options:MusicXmlImportOptions):Readonly<NotationMusicXmlImportResult>=>{
  const runtime=createMusicXmlProcessingRuntime(options);const parsed=parseMusicXmlTree(input,runtime);
  ...
  const core=scoreOnlyProjection(root);const coreBytes=new TextEncoder().encode(core).byteLength;
  ...
  const notation=buildNotation(score,root);
  const measureSemantics=createMusicXmlMeasureSemanticsDocument(score,base.measureSemantics);
