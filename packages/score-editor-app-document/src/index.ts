...
export interface NewAppDocumentOptions { readonly title?:string; readonly idFactory?:()=>string; readonly rendererProfile?:RendererProfile }
export interface OpenMusicXmlAppDocumentOptions { readonly title?:string; readonly documentId?:string; readonly revisionId?:string; readonly sha256Hex?:AppSha256Provider; readonly rendererProfile?:RendererProfile }
...
const sessionFromV3=(score:Parameters<typeof createEditorSessionV4FromV3>[0],notation:Parameters<typeof createEditorSessionV4FromV3>[1],profile:RendererProfile|undefined):Readonly<EditorSessionStateV4>=>profile===undefined?createEditorSessionV4FromV3(score,notation):createEditorSessionV4FromV3WithRendererProfile(score,notation,profile);
export const createNewScoreEditorAppDocument=(options:NewAppDocumentOptions={}):Readonly<ScoreEditorAppDocument>=>{const factory=options.idFactory??defaultIdFactory;const pair=blankV2(factory);const migrated=migrateScoreNotationV2ToV3(pair.score,pair.notation);const session=sessionFromV3(migrated.score,migrated.notation,options.rendererProfile);return appState(cleanTitle(options.title,'Untitled'),'NEW',session,null);};
export const openMusicXmlScoreEditorAppDocument=async(musicXml:string,options:OpenMusicXmlAppDocumentOptions={}):Promise<Readonly<ScoreEditorAppDocument>>=>{const bytes=new TextEncoder().encode(musicXml);const source=Object.freeze({sha256:await verifiedSha256Hex(musicXml,options.sha256Hex??browserSha256Hex),format:'musicxml' as const,byteLength:bytes.byteLength});const imported=importNotationMusicXmlV2(musicXml,{source,...(options.documentId===undefined?{}:{documentId:options.documentId}),...(options.revisionId===undefined?{}:{revisionId:options.revisionId})});const migrated=migrateScoreNotationV2ToV3(imported.score,imported.notation);const session=sessionFromV3(migrated.score,migrated.notation,options.rendererProfile);return appState(cleanTitle(options.title,'Imported Score'),'MUSICXML',session,session.history.present.score.revision.id);};
...
export const selectAppSemanticAddress=(document:ScoreEditorAppDocument,address:SemanticAddressV3|null):Readonly<ScoreEditorAppDocument>=>appState(document.title,document.origin,selectSessionSemanticAddressV4(document.session,address),document.savedRevisionId);
...
export const commitAppKeypadAction=(document:ScoreEditorAppDocument,action:unknown,advancedTarget:unknown,options:EditorKeypadV4Options):Readonly<ScoreEditorAppDocument>=>appState(document.title,document.origin,commitSessionKeypadActionV4(document.session,action,advancedTarget,options),document.savedRevisionId);
