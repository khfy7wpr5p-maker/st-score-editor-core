...
const blankV2 = (factory: () => string): Readonly<{ score: Readonly<ScoreDocumentV2>; notation: ReturnType<typeof createNotationDocumentV2> }> => {
  ...
  const staffId = id(factory, 'staff');
  const measureId = id(factory, 'measure');
  const voiceId = id(factory, 'voice');
  const restId = id(factory, 'event');
      ...
      staves: [{
        ...
        measures: [{
          ...
          voices: [{
            id: voiceId,
            ordinal: 1,
            events: [{ id: restId, kind: 'rest', onset: { numerator: 0, denominator: 1 }, duration: { numerator: 1, denominator: 1 } }],
            graceGroups: []
          }]
        }]
      }]
...
export const createNewScoreEditorAppDocument = (
  ...
  const pair = blankV2(factory);
  const migrated = migrateScoreNotationV2ToV3(pair.score, pair.notation);
  const session = createEditorSessionV4FromV3(migrated.score, migrated.notation);
...
export const commitAppCrossStaffIntent = (
  ...
  document.title,
  document.origin,
  commitSessionCrossStaffIntentV4(document.session, intent, options),
...
export const commitAppTopologyIntent = (
  ...
  document.title,
  document.origin,
  commitSessionTopologyIntentV4(document.session, intent, options),
  document.savedRevisionId
);
...
export const navigateAppDocumentHistory = (
  ...
  document.title,
  document.origin,
  navigateSessionHistoryV4(document.session, direction),
  document.savedRevisionId
);
