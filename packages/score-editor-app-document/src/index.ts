...
export const NEW_SCORE_PRESETS = Object.freeze([
  'LEGACY_PIANO_SINGLE_STAFF',
  'GUITAR_TREBLE',
  'PIANO_GRAND_STAFF'
] as const);
...
export interface NewAppDocumentOptions {
  readonly title?: string;
  readonly idFactory?: () => string;
  readonly rendererProfile?: RendererProfile;
  readonly preset?: NewScorePreset;
}

export interface OpenMusicXmlAppDocumentOptions {
  readonly title?: string;
...
export class ScoreEditorAppDocumentError extends Error {
  readonly code: ScoreEditorAppDocumentErrorCode;
  readonly details: Readonly<Record<string, unknown>>;
  constructor(message: string, code: ScoreEditorAppDocumentErrorCode, details: Record<string, unknown> = {}) {
    super(message);
    this.name = 'ScoreEditorAppDocumentError';
    this.code = code;
...
const cleanTitle = (value: string | undefined, fallback: string): string => {
  ...
  if (title !== title.trim() || title.length === 0 || title.length > 256) {
    throw new ScoreEditorAppDocumentError('Document title must be a trimmed non-empty string up to 256 characters.', 'INVALID_TITLE');
  }
  return title;
};

const cleanPreset = (value: NewScorePreset | undefined): NewScorePreset => {
  const preset = value ?? 'LEGACY_PIANO_SINGLE_STAFF';
  if (!(NEW_SCORE_PRESETS as readonly string[]).includes(preset)) {
    throw new ScoreEditorAppDocumentError('New score preset is unsupported.', 'INVALID_PRESET', { preset });
  }
  return preset;
};
...
const defaultIdFactory = (): string => {
  ...
  if (cryptoValue === undefined || typeof cryptoValue.randomUUID !== 'function') {
    throw new ScoreEditorAppDocumentError('Browser-safe randomUUID support is required to create a new app document.', 'ID_FACTORY_UNAVAILABLE');
  }
...
const appState = (
  ...
  session,
  savedRevisionId,
  dirty: savedRevisionId === null || session.history.present.score.revision.id !== savedRevisionId
});

type BlankStaffPlan = Readonly<{
  staffId: string;
  measureId: string;
  voiceId: string;
  restId: string;
  clef: Readonly<{ sign: 'G' | 'F'; line: number; octaveChange: number }>;
}>;

const blankStaffPlan = (
  factory: () => string,
  clef: BlankStaffPlan['clef']
): BlankStaffPlan => Object.freeze({
  staffId: id(factory, 'staff'),
  measureId: id(factory, 'measure'),
  voiceId: id(factory, 'voice'),
  restId: id(factory, 'event'),
  clef: Object.freeze({ ...clef })
});

const blankV2 = (
  factory: () => string,
  preset: NewScorePreset
): Readonly<{ score: Readonly<ScoreDocumentV2>; notation: ReturnType<typeof createNotationDocumentV2> }> => {
  const documentId = id(factory, 'doc');
  const revisionId = id(factory, 'rev');
  const partId = id(factory, 'part');
  const partName = preset === 'GUITAR_TREBLE' ? 'Guitar' : 'Piano';
  const staffPlans: readonly BlankStaffPlan[] = preset === 'PIANO_GRAND_STAFF'
    ? Object.freeze([
        blankStaffPlan(factory, { sign: 'G', line: 2, octaveChange: 0 }),
        blankStaffPlan(factory, { sign: 'F', line: 4, octaveChange: 0 })
      ])
    : Object.freeze([
        blankStaffPlan(factory, { sign: 'G', line: 2, octaveChange: 0 })
      ]);
  ...
  const score = createScoreDocumentV2({
    ...
    source: { sha256: '0'.repeat(64), format: 'synthetic', byteLength: null },
    parts: [{
      id: partId,
      name: partName,
      staves: staffPlans.map((plan, index) => ({
        id: plan.staffId,
        ordinal: index + 1,
        measures: [{
          id: plan.measureId,
  ...
  const measures = staffPlans.map((plan) => {
    const measureTarget = addressEntityV2(score, plan.measureId);
...
const sessionFromV3 = (
  score: Parameters<typeof createEditorSessionV4FromV3>[0],
  notation: Parameters<typeof createEditorSessionV4FromV3>[1],
...
export const createNewScoreEditorAppDocument = (
  ...
  const preset = cleanPreset(options.preset);
  const pair = blankV2(factory, preset);
  const migrated = migrateScoreNotationV2ToV3(pair.score, pair.notation);
  const session = sessionFromV3(migrated.score, migrated.notation, options.rendererProfile);
  return appState(cleanTitle(options.title, 'Untitled'), 'NEW', session, null);
};

export const openMusicXmlScoreEditorAppDocument = async (
  musicXml: string,
...
export const exportMusicXmlScoreEditorAppDocument = (document: ScoreEditorAppDocument): string => {
  try {
    return renderableMusicXmlV4(document.session.renderRequest);
  } catch (error) {
...
export const markScoreEditorAppDocumentSaved = (
  document: ScoreEditorAppDocument,
  title: string = document.title
): Readonly<ScoreEditorAppDocument> => appState(
  cleanTitle(title, document.title),
  document.origin,
  document.session,
  document.session.history.present.score.revision.id
);

export const selectAppSemanticAddress = (
  document: ScoreEditorAppDocument,
  address: SemanticAddressV3 | null
): Readonly<ScoreEditorAppDocument> => appState(
  document.title,
  document.origin,
  selectSessionSemanticAddressV4(document.session, address),
  document.savedRevisionId
);

export const commitAppBasicAuthoringIntent = (
  document: ScoreEditorAppDocument,
  intent: unknown,
  options: BasicAuthoringV4Options
): Readonly<ScoreEditorAppDocument> => appState(document.title, document.origin, commitSessionBasicAuthoringIntentV4(document.session, intent, options), document.savedRevisionId);

export const commitAppGraceAuthoringIntent = (
  document: ScoreEditorAppDocument,
  intent: unknown,
  options: GraceAuthoringV4Options
): Readonly<ScoreEditorAppDocument> => appState(document.title, document.origin, commitSessionGraceAuthoringIntentV4(document.session, intent, options), document.savedRevisionId);

export const commitAppArticulationAuthoringIntent = (
  document: ScoreEditorAppDocument,
  intent: unknown,
  options: ArticulationAuthoringV4Options
): Readonly<ScoreEditorAppDocument> => appState(document.title, document.origin, commitSessionArticulationAuthoringIntentV4(document.session, intent, options), document.savedRevisionId);

export const commitAppOrnamentAuthoringIntent = (
  document: ScoreEditorAppDocument,
  intent: unknown,
  options: OrnamentAuthoringV4Options
): Readonly<ScoreEditorAppDocument> => appState(document.title, document.origin, commitSessionOrnamentAuthoringIntentV4(document.session, intent, options), document.savedRevisionId);

export const commitAppKeypadAction = (
  document: ScoreEditorAppDocument,
  action: unknown,
...
): Readonly<ScoreEditorAppDocument> => appState(document.title, document.origin, commitSessionCrossStaffIntentV4(document.session, intent, options), document.savedRevisionId);

export const commitAppTopologyIntent = (
  document: ScoreEditorAppDocument,
  intent: unknown,
  options: TopologyAuthoringV3Options
): Readonly<ScoreEditorAppDocument> => appState(document.title, document.origin, commitSessionTopologyIntentV4(document.session, intent, options), document.savedRevisionId);

export const navigateAppDocumentHistory = (
  document: ScoreEditorAppDocument,
  direction: 'UNDO' | 'REDO'
): Readonly<ScoreEditorAppDocument> => appState(document.title, document.origin, navigateSessionHistoryV4(document.session, direction), document.savedRevisionId);
