...
const hasGraceEventNotation=(value:GraceEventNotationV2):boolean=>value.slash||value.dots!==0||value.beams.length>0||value.articulations.length>0||value.ornaments.length>0;
const hasGraceNoteNotation=(value:GraceNoteNotationV2):boolean=>value.accidental!==null||value.ties.length>0||value.slurs.length>0;
...
const retainGraceNotation=(extra:VoiceExtras,item:GraceParsedEvent):void=>{if(hasGraceEventNotation(item.eventNotation))extra.graceEvents.push({id:item.event.id,notation:item.eventNotation});extra.graceNotes.push(...item.noteNotations.filter((entry)=>hasGraceNoteNotation(entry.notation)));};
