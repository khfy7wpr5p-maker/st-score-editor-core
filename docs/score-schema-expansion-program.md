...
Status: **SSE-06 COMPLETE / MERGE CANDIDATE**

The vNext contract frozen by SSE-00 is now implemented through bounded MusicXML v2 semantic round trip. SSE-07 is the next autonomous stage. Staff/part topology remains a separate human-gated design at SSE-08.
...
- no new runtime dependency is required by SSE-00–06;
...
- **SSE-06 — COMPLETE / MERGE CANDIDATE:** isolated bounded MusicXML v2 import/export round trip.
...
SSE-06 keeps every legacy v1 MusicXML parser/importer unchanged. `packages/musicxml-v2` adds a separate bounded safe parser plus `serializeNotationMusicXmlV2` and `importNotationMusicXmlV2`.
...
Admitted round-trip semantics include grace note/rest/chord content, written value and bounded playback metadata; normal/grace articulations; simple ornaments and accidental marks; single/spanning tremolo; and wavy-line start/continue/stop. Sparse default notation remains sparse.
...
5. prove bounded v2 MusicXML round trip without widening v1 — SSE-06 complete on this merge candidate;
6. wire renderer/SesliTab to the proven v2 projection while retaining opaque semantic identity — SSE-07 next.

Direct v2 MusicXML exchange now exists, but current renderer requests may remain `VNEXT_XML_PENDING` for v2-only pairs until SSE-07. This is a renderer-integration boundary, not missing MusicXML round-trip capability.

## Definition of done

The schema-expansion program through SSE-07 is complete only after renderer/SesliTab v2 compatibility is proven without authority drift. Staff/part topology and cross-staff remain separate later gates.