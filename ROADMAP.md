...
Updated: **2026-09-15**  
Repository source of truth: `main` at **`e281d52881aaad0fade574a944251b12ea752038`**.
...
- **P07 latency hardening — merged** for non-blocking note selection / audition behavior.
...
- **P08 qualified Violin integration — COMPLETE / MERGED via PR #176**: Grand Piano + Violin in the default editor audition host, with Grand Piano still default and Classical Guitar still suspended.

## Current professional browser milestone

`STScoreEditorProfessionalApp` is an independently built optional artifact rather than an expansion of the default app bundle.

Qualification facts:

- professional artifact merge: `0adabbdc426f8481196ee41cce5c9b2bbff2768e`;
- current main after Violin integration: `e281d52881aaad0fade574a944251b12ea752038`;
...
- Node 18/20/22 exact-head tests for P08-E4: PASS;
- dedicated professional WebKit regression: PASS;
- retained APP-10/11, APP-09B and P05 WebKit regressions: PASS.

This is **not** a physical iPhone PASS and does **not** authorize production exposure of the professional artifact.

## Current default-editor audio state

- Grand Piano: qualified and default.
- Violin: qualified and merged through PR #176 using Audio Engine v0.1.2.
- Classical Guitar: suspended and absent from the selector.
- Audio selection is host-only state and cannot mutate canonical score/history/renderer state.
- The professional P08-E4 artifact still does not bundle or integrate the audio host.
...
- Undo/Redo semantics;
- P08 professional selection interactions;
...
- keyboard and pointer/touch edits converge on the same mutation engines;
...
- Keep Grand Piano + Violin behavior stable in the default editor.
- Keep Classical Guitar suspended.
- Do not use P09 as a reason to add audio to the professional artifact.
- Any professional-artifact audio composition requires a separate bounded architecture/qualification stage.
...
- audio admission/composition regressions whenever shared audio-host code changes;
- dedicated P09 command tests once P09 implementation begins.

## Current release/product flags

```text
currentMain = e281d52881aaad0fade574a944251b12ea752038
...
defaultEditorGrandPianoQualified = true
defaultEditorViolinQualified = true
classicalGuitarSuspended = true
